...
std::vector<std::string> Sort::sortFile(const std::string& sortType, std::vector<std::string>& logEntries) {
    if (logEntries.empty()) {
        std::cerr << "Error: Log entries are empty.\n";
        return logEntries;
    }

    auto compareTimestamp = [](const std::string& logEntry1, const std::string& logEntry2) {
