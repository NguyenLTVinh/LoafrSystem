...
protected:
    ...
    void SetUp() override {
        logEntries = {
            "2023-11-14T09:10:00, insulin pump, Start-Infusion, infusion-amount, 10",
            "2023-11-14T09:15:00, patient simulator, glucose-monitor-report, sugar-level, 125",
...
TEST_F(SearchTest, KeywordPresentMultipleTimes) {
    std::vector<std::string> result = search.searchKeyword("insulin pump", logEntries);
...
TEST_F(SearchTest, KeywordPresentOneTime) {
    std::vector<std::string> result = search.searchKeyword("heart-rate", logEntries);
...
TEST_F(SearchTest, KeywordMatchingFieldName) {
    std::vector<std::string> result = search.searchKeyword("infusion-amount", logEntries);
    ASSERT_EQ(result.size(), 2u);
}
